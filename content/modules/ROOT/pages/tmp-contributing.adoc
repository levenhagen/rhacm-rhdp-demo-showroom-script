...
. Test locally using a standalone playbook if possible (see xref:tmp-ocp4-role-reference.adoc[OCP4 examples] or xref:tmp-vm-role-reference.adoc[VM examples]).
...
. Test locally with Podman (see xref:tmp-content-repo.adoc[]).
